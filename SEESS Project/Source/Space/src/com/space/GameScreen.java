 ...
 * All images displayed on the screen are referenced from Space.
 * All game logic that depends on framecounts are initiated here, any in depth 
 * manipulation of the Player class is handled by the player class.
 * This screen moves to the NewWaveShipScreen once a wave is finished.
 ****************************************************************************/
...
public class GameScreen implements Screen {
	...
	// This class handles rendering and updating all objects on the gameScreen
	...
	// Enemies and enemy projectiles are stored and managed in ArrayLists
	
	double angle = 0;
	
		Space imageLib; //Reference handle for image library
		...
		//Basic textures needed for rendering on this screen
		Texture blankImg;
		Texture shipImg;
		Texture enemyImg;
	   ...
	   //The array lists of all elements managed on this screen
	...
	public GameScreen(Space game) {
		...
		//Copy over all necessary images
		this.imageLib = game;
		...
		/*Yea, had to comment this out. The sound effects were driving me crazy during testing
		// load the laser sound effect and the background "music"
	      ...
	      */
	      ...
	      //Two backgrounds placed end to end. (They synchronously scroll to create nifty effect)
	...
	private void update() {
		// This is where all the fancy game magic happens.
		
		currentFrame= currentFrame+1%60; //Keep track of frames in set of 60. (60fps)
		explosionFrame++; //Increment the current frame of explosion. (Animation not implemented yet)
	    ...
	    //Debugging Cheats
	    if(Gdx.input.isKeyPressed(Keys.R)){Space.player.currentForm = -1; Space.player.health = Space.player.healthMax;} //Come back to life
	    if(Gdx.input.isKeyPressed(Keys.A)){ //Print out all player stats
	    	//Printout
	   ...
	   //Test Explosions Update Loop
	     ...
	     //Update the position of each explosion
	     for(int i = 0; i<testExplosions.size(); i++){
	    	 ...
	    	 if(testExplosions.get(i).getRect().x + 48 < 0||testExplosions.get(i).getRect().y>480) {testExplosions.remove(i);} 
	      }
	...
	public void render(float delta) {
	    ...
	    for(int i = 0; i<500; i++)  Gdx.graphics.getDeltaTime();
	      ...
	      /** Draw Function
	       * batch.draw(TextureImg,X-positionToDraw, Y-PositionToDraw);
	       */
		  ...
		  //Draw Space.player
		    ...
		    //Draw necessary information to screen using font
